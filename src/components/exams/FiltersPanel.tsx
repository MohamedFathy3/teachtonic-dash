...
// src/components/exams/FiltersPanel.tsx
...
import { GraduationCap, BookOpen, Calendar, Power, DollarSign, Search, X } from 'lucide-react';
          ...
          <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t">
            ...
            {filters.stageId && (
              <Badge variant="secondary" className="gap-1 text-xs">
                <GraduationCap className="h-3 w-3" /> {t('stage')}
              </Badge>
            )}
            {filters.lessonId && (
              <Badge variant="secondary" className="gap-1 text-xs">
                <BookOpen className="h-3 w-3" /> {t('lesson')}
              </Badge>
            )}
            {filters.active !== null && (
              <Badge variant="secondary" className="gap-1 text-xs">
                <Power className="h-3 w-3" /> {filters.active ? t('active') : t('inactive')}
              </Badge>
            )}
            {filters.marksMin && (
              <Badge variant="secondary" className="gap-1 text-xs">
