...
import { QuestionBankModal } from './QuestionBankModal'; // ✅ استيراد المودال المنفصل
import { 
  Loader2, 
  Plus, 
  Trash2, 
...
export const QuestionBuilder: React.FC<QuestionBuilderProps> = ({ examId, onSuccess, onCancel }) => {
  ...
  const [questionForm, setQuestionForm] = useState<Question>({
    ...
    image: null,
    options: [
      { option_text: '', is_correct: false, image: 0 },
      { option_text: '', is_correct: false, image: 0 },
      { option_text: '', is_correct: false, image: 0 },
      { option_text: '', is_correct: false, image: 0 },
    ],
  ...
  const removeOptionImage = (index: number) => {
    const newOptions = [...(questionForm.options || [])];
    newOptions[index] = { ...newOptions[index], image: 0 };
    setQuestionForm({ ...questionForm, options: newOptions });
  };
  ...
  const addOption = () => {
    setQuestionForm({
      ...questionForm,
      options: [...(questionForm.options || []), { option_text: '', is_correct: false, image: 0 }],
    });
  };
        ...
        <div className="flex w-full items-start justify-between gap-4 flex-nowrap min-w-0">
          ...
          <div className="flex gap-2">
            <Button 
              ...
              variant="secondary"
              className="flex items-center gap-2 px-5 py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg hover:shadow-xl transition-all"
            >
        ...
        {questions.length === 0 ? (
          <Card className="p-16 text-center">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              <Button variant="link" onClick={openBankModal} className="text-indigo-500">
          ...
          <div className="space-y-4">
            <AnimatePresence mode="popLayout">
              {questions.map((q, idx) => (
                ...
                >
                  <Card 
                    className="p-5 border-2 hover:border-primary/50 transition-all shadow-lg cursor-pointer hover:shadow-xl" 
          ...
          <div className="p-6 space-y-6">
            ...
            {questionForm.question_type === 'multiple_choice' && (
              <div>
                ...
                <div className="space-y-4 p-4 bg-muted/30 rounded-xl max-h-60 overflow-y-auto">
